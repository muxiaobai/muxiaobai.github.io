...
一个BM25算分过程
是在query过程中完成，match或者phrase过程都是一个算分过程

例如搜索了摆脱贫困，拆分为 摆脱和贫困两个词，然后在 title、intro和content等字段中查找，有命中的情况就会返回到数据。
下面的计算过程是摆脱在content字段的计算过程，其它同理。
Score= boost * idf * tf
tf = freq / (freq + k1 * (1 - b + b * dl / avgdl))
Idf = log(1 + (N - n + 0.5) / (n + 0.5))

k1 = 1.2
boost =  (k1+1)*该字段的boost，字段权重
tf 词频(Term Frequency) 一般情况下，词频越高，那么就越相关

freq为该文档中摆脱出现的次数，
k1 =1.2 缺省值，词饱和度值
b = 0.75 缺省值，长度归一化参数
dl field length该文档的长度 
avgdl 平均字段的长度

Idf逆文本频率指数(Inverse Document Frequency)，此指标认为，如果所有的文档中都出现该检索词，那么这个词就没那么重要，因为类似均分，平均水平提高了，这个参数就会降低。
其中N为全部的文档数，n为包含摆脱的文档数
